...
int main() {
    ...
    while (test--) {
        ...
        string s; cin >> s;
        int o = s.size();
        string d = s; reverseStr(d);
        s = s + d;
        int l1, r1, l2, r2; //cin >> l1 >> r1 >> l2 >> r2;
        char cmin = *min_element(s.begin(), s.end());
        RH_double rh(s, cmin); // O(s.size())
        int pal = 0;
        //comparacion entre todos los sub strings de tamano n en el string
        //Aplicacion de rolling hash para un substring y su "inverso"
        for (int i = 0; i <= o - n; ++i)
        {
            l1 =  i; r1 = l1 + n - 1;
            r2 = s.size() - 1 - i;l2 = r2 - n + 1;
            ull h1 = rh.hash(l1, r1); // O(1)
            ull h2 = rh.hash(l2, r2); // O(1)
            string s1 = s.substr(l1, r1-l1+1);
            string s2 = s.substr(l2, r2-l2+1);
            if(h1 == h2)pal++;//contador de cantidad de palindromos
...
Rabin-Karp algorythm: rescatado de clase practica
*/